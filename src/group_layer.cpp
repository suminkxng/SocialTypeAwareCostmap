...
{
void GroupLayer::onInitialize()
{
  ros::NodeHandle nh("~/" + name_), g_nh;
  current_ = true;
  first_time_ = true;
  people_sub_ = nh.subscribe("/group_detection", 1, &GroupLayer::peopleCallback, this);
}

void GroupLayer::peopleCallback(const hri_msgs::PersonArray& people)
{
...
void GroupLayer::updateBounds(double origin_x, double origin_y, double origin_z, double* min_x, double* min_y,
                                double* max_x, double* max_y)
{
  ...
  {
    hri_msgs::Person& person = people_list_.people[i];
    hri_msgs::Person tpt;
